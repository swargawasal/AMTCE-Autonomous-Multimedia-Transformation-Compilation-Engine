...
logger = logging.getLogger("AkankshaV5")

# --- CONFIG ---
INPUT_VIDEO = r"D:\Autonomous Multimedia Transformation & Compilation Engine (AMTCE)\downloads\Akanksha_puri.mp4"
TEMP_DIR = os.path.join(root_dir, "temp", "akanksha_test")
OUTPUT_VIDEO = os.path.join(root_dir, "final_akanksha_v5.mp4")
...
async def run_test():
    ...
    logger.info(f"🧠 Running V5.1 Engine for Akanksha Puri...")
    ...
    logger.info("🛠️ Building Multi-Line Karaoke Filter...")
    chunk_size = 5 
    ...
    for chunk in chunks:
        ...
        for line_idx, line_text in enumerate(wrapped_lines):
            ...
            filters.append(f"{last_label}drawtext=fontfile='{safe_font}':text='{line_text}':fontsize={STABLE_FONT_SIZE}:fontcolor=white:borderw=5:bordercolor=black:shadowx=2:shadowy=2:x={start_x}:y={y_pos}:enable='between(t,{chunk_start:.3f},{chunk_end:.3f})'{out_label_base};")
        ...
        for w_info, line_idx, abs_x in words_on_lines:
            ...
            filters.append(f"{last_label}drawtext=fontfile='{safe_font}':text='{word_text}':fontsize={STABLE_FONT_SIZE}:fontcolor=yellow:borderw=6:bordercolor=black:shadowx=3:shadowy=3:x={abs_x}:y={y_pos}:enable='between(t,{w_start:.3f},{w_end:.3f})'{out_label_highlight};")
            last_label = out_label_highlight
            idx += 1

    filters.append(f"{last_label}null[v_final]")
    with open(os.path.join(TEMP_DIR, "karaoke.txt"), "w", encoding="utf-8") as f:
        f.write("\n".join(filters))

    logger.info("🎬 Rendering Akanksha V5.1...")
    cmd = ["ffmpeg", "-y", "-i", INPUT_VIDEO, "-i", audio_path, "-filter_complex_script", os.path.join(TEMP_DIR, "karaoke.txt"), "-map", "[v_final]", "-map", "1:a", "-c:v", "libx264", "-preset", "ultrafast", "-crf", "18", "-c:a", "aac", "-b:a", "192k", OUTPUT_VIDEO]
